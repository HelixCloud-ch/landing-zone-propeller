variable "vpc_id" {
  type        = string
  description = "ID of the hub VPC the subnets are created in."
...
variable "vpc_cidr" {
  type        = string
  description = "IPv4 CIDR block of the hub VPC. Subnet CIDRs are carved from this block. Must be a valid IPv4 CIDR (e.g. 10.0.0.0/16)."
...
variable "tiers" {
  ...
  description = <<-EOT
    ...
    should be true only for the public tier.
  EOT
...
variable "name_prefix" {
  type        = string
  description = "Prefix applied to the Name tag of every subnet (e.g. \"network-hub\" yields \"network-hub-private-<az>\")."
