...
locals {
  ...
  # Collector config is templated as a single YAML string injected via Helm values.
  # cluster_name and region are interpolated at plan time — no env-var indirection
  # needed because Terraform owns the deployment.
