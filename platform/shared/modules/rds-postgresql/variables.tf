...
variable "engine" {
  ...
  validation {
    ...
    error_message = "Engine must be 'postgres'. For Aurora use the aurora-postgresql module."
  }
}
...
variable "engine_version" {
  ...
  validation {
    ...
    error_message = "engine_version must be a numeric version string (e.g. '16' or '16.4')."
  }
}
...
variable "enabled_cloudwatch_logs_exports" {
  type        = list(string)
  description = "List of log types to export to CloudWatch. Valid values for PostgreSQL: 'postgresql', 'upgrade'."
  default     = ["postgresql", "upgrade"]

  validation {
    condition     = alltrue([for l in var.enabled_cloudwatch_logs_exports : contains(["postgresql", "upgrade"], l)])
    error_message = "enabled_cloudwatch_logs_exports entries must be 'postgresql' and/or 'upgrade'."
  }
}
