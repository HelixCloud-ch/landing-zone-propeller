...
| <a name="input_role_name"></a> [role\_name](#input\_role\_name) | Name of the IRSA role for the controller. Defaults to '<cluster\_name>-aws-load-balancer-controller' when null. Override only when the naming convention conflicts with an existing role or IAM path constraint. | `string` | `null` | no |
