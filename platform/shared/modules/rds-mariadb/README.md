...
Shared Terraform module for RDS MariaDB instances. Used by the
`rds-mariadb` platform project.
...
## MariaDB-specific constraints

| Parameter | Constraint |
|-----------|-----------|
| `db_name` | 1-64 chars, starts with a letter, then letters/digits/underscores |
| `username` | 1-16 chars, starts with a letter, letters and digits only |
| `password` | 8-41 chars, printable ASCII excluding `/`, `'`, `"`, `@`, space |
| `log exports` | `audit`, `error`, `general`, `slowquery` |
| `default port` | 3306 |

## What does NOT belong here

- Aurora MySQL/MariaDB clusters — use a separate aurora module.
- MySQL (non-MariaDB) instances — use the `rds-mysql` module if created.
- Option groups — MariaDB does not support option groups.

## References

- [RDS MariaDB naming and password constraints](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/CHAP_Limits.html)
- [Publishing MariaDB logs to CloudWatch Logs](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/USER_LogAccess.MariaDB.PublishtoCloudWatchLogs.html)
- [aws_db_instance Terraform resource](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/db_instance)
...
| <a name="input_db_name"></a> [db\_name](#input\_db\_name) | Name of the initial database to create. Must begin with a letter; subsequent characters can be letters, underscores, or digits (1-64 chars). | `string` | `"appdb"` | no |
...
| <a name="input_enabled_cloudwatch_logs_exports"></a> [enabled\_cloudwatch\_logs\_exports](#input\_enabled\_cloudwatch\_logs\_exports) | List of log types to export to CloudWatch. Valid values for MariaDB: 'audit', 'error', 'general', 'slowquery'. | `list(string)` | <pre>[<br/>  "error",<br/>  "slowquery"<br/>]</pre> | no |
| <a name="input_engine"></a> [engine](#input\_engine) | RDS engine name. | `string` | `"mariadb"` | no |
| <a name="input_engine_version"></a> [engine\_version](#input\_engine\_version) | MariaDB major version (e.g. '10.11'). Minor version is auto-selected when auto\_minor\_version\_upgrade is true. | `string` | `"10.11"` | no |
...
| <a name="input_password"></a> [password](#input\_password) | Master password. Used only when master\_user\_secret\_kms\_key\_id is not set. Mutually exclusive with it. Ephemeral: never written to state or plan (delivered via the write-only password\_wo argument). MariaDB allows 8-41 printable ASCII characters excluding '/', single-quote, '"', '@', and spaces. | `string` | `null` | no |
...
| <a name="input_port"></a> [port](#input\_port) | Port the MariaDB server listens on. | `number` | `3306` | no |
...
| <a name="input_username"></a> [username](#input\_username) | Master username for the database. MariaDB allows 1-16 letters or numbers, must start with a letter. | `string` | `"dbadmin"` | no |
